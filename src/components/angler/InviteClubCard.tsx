...
interface InviteClubCardProps {
  existingInvitations: Invitation[];
  hasMemberships: boolean;
}

export default function InviteClubCard({
  existingInvitations,
  hasMemberships,
}: InviteClubCardProps) {
  ...
  // Don't show the card if the angler already has active memberships
  // and no pending invitations
  if (hasMemberships && invitations.length === 0) {
    return null;
  }
    ...
    <Card className="border-river/20 bg-river/5">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-lg">
          ...
          {hasMemberships ? "Invite Another Club" : "Connect With Your Club"}
        </CardTitle>
        <CardDescription>
          {hasMemberships
            ? "Invite another club you belong to so you can book through them too."
            : "Your membership activates when your club joins AnglerPass. Invite your club admin to get started — it only takes a minute."}
        </CardDescription>
      </CardHeader>
