...
/** Simple markdown-to-HTML converter for headings, paragraphs, lists, bold, links */
    ...
    .map((block) => {
      ...
      if (trimmed.startsWith('- ')) {
        const items = trimmed
          ...
          .map((l) => `<li class="relative pl-5 before:content-[''] before:absolute before:left-0 before:top-[0.55em] before:w-[7px] before:h-[7px] before:bg-forest before:clip-path-diamond before:opacity-50">${inline(l.slice(2))}</li>`)
          .join('');
