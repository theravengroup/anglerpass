  ...
  if (!data) {
    return (
      <div className="py-20 text-center text-text-secondary">
        Unable to load affiliate data. The affiliate tables may not be set up yet.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3">
        <div className="flex size-10 items-center justify-center rounded-xl bg-bronze/10 text-bronze">
          <Link2 className="size-5" />
        </div>
        <div>
          <h1 className="font-heading text-xl font-semibold text-forest-deep">
            Affiliate Program
          </h1>
          <p className="text-sm text-text-secondary">
            Compass AI product recommendations · {data.period}
          </p>
        </div>
      </div>
        ...
        {STAT_CARDS.map((card) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-xs text-text-light mt-0.5">{card.label}</p>
            </div>
          );
        })}
      </div>
        ...
        <div className="rounded-xl border border-parchment bg-white">
          ...
          <div className="divide-y divide-parchment/50">
            {topProducts.length === 0 ? (
              <p className="px-5 py-8 text-center text-sm text-text-light">
                No click data yet. Product clicks will appear here once Compass AI starts recommending products.
              </p>
            ) : (
      ...
      <div className="max-w-2xl">
        <h3 className="mb-2 font-heading text-sm font-semibold text-forest-deep">
          Affiliate Disclosure (shown to users)
        </h3>
